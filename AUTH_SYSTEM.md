# 注册和认证系统实现文档

## 📋 功能概述

本项目已实现完整的注册登录系统，包括：

1. ✅ 基于激活码的用户注册
2. ✅ 用户登录认证
3. ✅ 管理员后台系统
4. ✅ 激活码管理功能
5. ✅ DeepSeek API 配置管理
6. ✅ 管理员看板统计

## 🔑 注册流程

### 用户注册
1. 用户访问 `/register` 注册页面
2. 填写：用户名、密码、确认密码、激活码
3. 系统验证：
   - 用户名唯一性（至少3个字符）
   - 密码强度（至少6个字符）
   - 激活码有效性（未使用、未过期）
4. 注册成功后跳转到用户画像采集页面

### 用户登录
1. 用户访问 `/login` 登录页面
2. 填写用户名和密码
3. 登录成功后：
   - 普通用户：跳转到 Dashboard 或 Onboarding
   - 管理员用户：跳转到管理员后台

## 🛡️ 管理员系统

### 创建初始管理员

**重要**：首次部署后需要手动创建管理员账号。

#### 方法 1：使用 Prisma Studio（推荐）

```bash
# 本地开发
npx prisma studio

# 然后在浏览器中：
# 1. 打开 users 表
# 2. 创建新用户
# 3. 设置字段：
#    - username: admin
#    - password: 8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918 (密码: admin，sha256哈希)
#    - isAdmin: true
#    - onboardingCompleted: true
```

#### 方法 2：直接执行 SQL

```sql
INSERT INTO users (username, password, isAdmin, onboardingCompleted, createdAt, updatedAt)
VALUES (
  'admin',
  '8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918',
  true,
  true,
  datetime('now'),
  datetime('now')
);
```

**默认管理员账号**：
- 用户名：`admin`
- 密码：`admin`（**部署后请立即修改**）

### 管理员功能

#### 1. 管理员看板 (`/admin`)
- 查看系统统计数据
- 总用户数、总会话数
- 激活码使用情况
- 最近注册用户列表
- 最近会话列表

#### 2. 激活码管理 (`/admin/activation-codes`)
- **生成激活码**
  - 批量生成（1-100个）
  - 设置备注说明
  - 设置有效期（可选）
- **查看激活码列表**
  - 激活码、状态、使用者
  - 备注、创建时间
  - 复制激活码功能
- **删除激活码**
  - 仅能删除未使用的激活码

#### 3. API 配置管理 (`/admin/settings`)
- 配置 DeepSeek API Key
- 所有用户对话将使用配置的 API
- 支持实时更新，无需重启

## 🔌 API 接口

### 认证相关

#### POST `/api/auth/register`
用户注册

**请求体**：
```json
{
  "username": "testuser",
  "password": "password123",
  "activationCode": "ABC123DEF456"
}
```

**响应**：
```json
{
  "success": true,
  "message": "注册成功",
  "user": {
    "id": 1,
    "username": "testuser",
    "isAdmin": false
  },
  "token": "base64_encoded_token"
}
```

#### POST `/api/auth/login`
用户登录

**请求体**：
```json
{
  "username": "testuser",
  "password": "password123"
}
```

**响应**：
```json
{
  "success": true,
  "message": "登录成功",
  "user": {
    "id": 1,
    "username": "testuser",
    "isAdmin": false,
    "onboardingCompleted": true
  },
  "token": "base64_encoded_token"
}
```

### 管理员接口

#### GET `/api/admin/dashboard?admin=username`
获取管理员看板数据

**响应**：
```json
{
  "stats": {
    "totalUsers": 10,
    "totalSessions": 25,
    "totalActivationCodes": 20,
    "usedActivationCodes": 10,
    "unusedActivationCodes": 10
  },
  "recentUsers": [...],
  "recentSessions": [...]
}
```

#### GET `/api/admin/activation-codes?admin=username`
获取激活码列表

#### POST `/api/admin/activation-codes`
生成激活码

**请求体**：
```json
{
  "adminUsername": "admin",
  "count": 5,
  "note": "销售部门",
  "expiresInDays": 30
}
```

#### DELETE `/api/admin/activation-codes?admin=username&id=1`
删除激活码

#### GET `/api/admin/settings?admin=username`
获取 API 配置

#### POST `/api/admin/settings`
更新 API 配置

**请求体**：
```json
{
  "adminUsername": "admin",
  "deepseekApiKey": "sk-..."
}
```

## 🗄️ 数据库变更

### 新增表

#### `activation_codes` - 激活码表
- `id`: 主键
- `code`: 激活码（唯一）
- `isUsed`: 是否已使用
- `usedBy`: 使用者ID
- `createdBy`: 创建者（管理员用户名）
- `note`: 备注
- `expiresAt`: 过期时间
- `usedAt`: 使用时间
- `createdAt`: 创建时间

#### `admin_logs` - 管理员操作日志表
- `id`: 主键
- `adminName`: 管理员用户名
- `action`: 操作类型
- `details`: 操作详情（JSON）
- `createdAt`: 操作时间

### 修改表

#### `users` 表增加字段
- `password`: 密码哈希（SHA256）
- `isAdmin`: 是否为管理员
- `activationCodeId`: 关联的激活码ID

#### `settings` 表
用于存储系统配置，特别是 `deepseek_api_key`

## 🚀 部署步骤

### 1. 数据库迁移

```bash
# 生成 Prisma Client
npx prisma generate

# 执行数据库迁移
npx prisma migrate deploy
# 或使用 db push（开发环境）
npx prisma db push
```

### 2. 创建管理员账号

使用上述方法创建初始管理员账号

### 3. 配置 DeepSeek API

1. 使用管理员账号登录
2. 进入"API 配置"页面
3. 输入 DeepSeek API Key
4. 保存配置

### 4. 生成激活码

1. 进入"激活码管理"页面
2. 点击"生成激活码"
3. 设置数量和备注
4. 复制激活码分发给用户

## 🔒 安全说明

### 密码安全
- 密码使用 SHA256 哈希存储
- 生产环境建议使用 bcrypt 或 argon2
- 建议实现密码复杂度策略

### Token 管理
- 当前使用简单的 Base64 编码
- 生产环境建议使用 JWT
- 建议实现 Token 过期机制

### API 安全
- 管理员接口使用用户名验证
- 建议实现更严格的认证中间件
- 建议添加 CSRF 保护

## 📝 使用流程

### 典型用户流程

1. **管理员准备**
   ```
   管理员登录 → 配置 API → 生成激活码 → 分发给用户
   ```

2. **用户注册**
   ```
   访问注册页 → 填写信息 → 输入激活码 → 注册成功 → 完成画像采集
   ```

3. **用户使用**
   ```
   登录 → 选择场景 → 开始对话 → 生成报告
   ```

### 管理员日常工作

1. **查看系统状态**
   - 每天查看管理员看板
   - 监控用户注册情况
   - 检查会话使用情况

2. **管理激活码**
   - 根据需求生成新激活码
   - 查看激活码使用情况
   - 清理过期激活码

3. **维护 API 配置**
   - 监控 API 使用额度
   - 必要时更新 API Key

## 🐛 故障排查

### 注册失败

**问题**：提示"激活码不存在"
- 检查激活码是否正确（大小写敏感）
- 确认激活码未被使用
- 检查激活码是否过期

**问题**：提示"用户名已存在"
- 尝试使用其他用户名

### 登录失败

**问题**：提示"用户名或密码错误"
- 确认用户名和密码正确
- 检查是否已完成注册

### 对话失败

**问题**：提示"DeepSeek API 未配置"
- 管理员需要在后台配置 API Key
- 检查 API Key 是否有效
- 确认 API 有足够额度

## 📞 技术支持

如有问题，请：
1. 查看浏览器控制台错误信息
2. 检查服务器日志
3. 联系技术团队

---

**安全提醒**：
- 部署后立即修改默认管理员密码
- 定期检查管理员操作日志
- 妥善保管激活码
- 不要将 API Key 泄露

**最佳实践**：
- 为不同部门生成带备注的激活码
- 设置合理的激活码有效期
- 定期清理未使用的过期激活码
- 监控 API 使用情况，避免超出额度


